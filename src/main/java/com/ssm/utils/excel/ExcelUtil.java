...
public class ExcelUtil {
	...
	public void readExcel() throws Exception {
		File file = new File("F:/csv/excel.xlsx");
		FileInputStream in = null;
		try {
			in = new FileInputStream(file);
			List<ExcelHead> excelHeads = null;
			
			// 如果表头和pojo名称不一致
			excelHeads = new ArrayList<ExcelHead>();
			ExcelHead head1 = new ExcelHead("userId","userId",true);
			ExcelHead head2 = new ExcelHead("姓名","userName");
			ExcelHead head3 = new ExcelHead("密码","userPassword");
			excelHeads.add(head1);
			excelHeads.add(head2);
			excelHeads.add(head3);
			
			List<User> list = ExcelUtil.readExcelToEntity(User.class, in, file.getName(), excelHeads);
			for (User excelUser : list) {
				System.out.println(excelUser.getUserId() + ":" + excelUser.getUserName() + ":" + excelUser.getUserPassword()) ;
			}
		} catch (Exception e) {
			e.printStackTrace();
		} finally {
